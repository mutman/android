...
public class BannerView extends WebView {
    ...
    private void initAds(AttributeSet attributeSet, Context context) {
        TypedArray a = context.obtainStyledAttributes(attributeSet, R.styleable.BannerView);
        ...
        for (int i = 0; i < N; ++i) {
            ...
            switch (attr) {
                case R.styleable.BannerView_serverHost:
                    ...
                    Log.d(BannerView.class.getSimpleName(), "found attribute: "+a.getString(attr));
                    break;
    ...
    private void initAds() {
        if (getServerHost().equals("")) {
            Log.e(BannerView.class.getSimpleName(), "You must specify serverHost attribute");
        } else if (getZoneId().equals("")) {
            Log.e(BannerView.class.getSimpleName(),"You must specify zoneid attribute");    
        } else {
            setVerticalScrollBarEnabled(false);
            setHorizontalScrollBarEnabled(false);
            getSettings().setJavaScriptEnabled(true);
            addJavascriptInterface(new BannerData(getZoneId(), getServerHost(), getRefreshInterval()), "bannerdata");
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(getMeasuredWidth() > 500){
	        setMeasuredDimension(500, getMeasuredHeight());
	    }
	}
    ...
    public void setServerHost(String serverHost) {
        StringBuilder baseURLBuilder = new StringBuilder(serverHost.length() + 8);
    ...
    public class BannerData {
        ...
        public BannerData(String zoneId, String serverHost, int refreshInterval) {
            ...
            this.refreshInterval = refreshInterval;
            Log.d(BannerView.class.getSimpleName(), "bannerdata: "+this.serverHost+", "+this.zoneId+", "+this.refreshInterval);
        }
